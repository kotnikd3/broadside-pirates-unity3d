...
public class Shooter : MonoBehaviour {
	...
	private float cooldownTmp;

	void Start()
	{
		cooldownTmp = cooldown;
	}
	...
	void Update () {
		if (cooldownTmp > 0) 
		{
			cooldownTmp = cooldownTmp - Time.deltaTime;
		}

		// Detect if fire button is pressed
		if ((Input.GetButtonDown("Fire1") || Input.GetButtonDown("Jump")) && cooldownTmp < 0 && !switchMode.firstPersonView)
		{	
			...
			if (projectile)
			{
				...
				newProjectile.GetComponent<Rigidbody>().AddForce(
					transform.forward * power + transform.up * 2.0f, ForceMode.VelocityChange);
